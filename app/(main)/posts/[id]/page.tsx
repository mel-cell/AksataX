...
import { useAppealPost } from "@/hooks/use-moderate-post";
import { CommentItem } from "@/components/ui/CommentItem";
import PostMenu from "@/components/ui/PostMenu";
import { PostModerationBanner } from "@/components/ui/PostModerationBanner";
import { PostAppealModal } from "@/components/ui/PostAppealModal";
...
export default function PostDetailPage() {
  ...
  const appealPost = useAppealPost();
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap mt-2.5 text-xs text-muted-foreground">
                  ...
                  <span className="ml-auto">
                    <PostMenu postId={post.id} postTitle={post.title} authorId={post.user.id} authorUsername={post.user.username} />
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="px-6 py-6">
            ...
            <div className="flex gap-2 flex-wrap mt-6 pt-5 border-t border-border">
              ...
              >
                <Bookmark
                  size={13}
                  className={post.is_bookmarked ? "fill-amber-500" : ""}
                />
        ...
        <div ref={commentsRef} className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
            ...
            <div className="flex gap-3 items-start">
              ...
              <div className="flex-1 flex flex-col gap-2">
                <textarea
                  ...
                  rows={3}
                  className="w-full resize-none rounded-xl border border-border bg-card text-sm px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-zinc-300 focus:border-zinc-400 transition placeholder:text-muted-foreground"
                />
      ...
      <PostAppealModal
        ...
        onSubmit={(reason) => {
          appealPost.mutate(
            ...
            { onSuccess: () => setShowAppealModal(false) },
          );
        }}
        isPending={appealPost.isPending}
      />
    </div>
  );
}
